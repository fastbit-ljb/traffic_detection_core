...
function BaselineControls({ config, onChange, compact = false }: { config: BaselineConfig; onChange: (config: BaselineConfig) => void; compact?: boolean }) {
  return <div className={compact ? 'baseline-controls model-baseline-controls' : 'baseline-controls'}>
    <label className="baseline-toggle"><input type="checkbox" checked={config.enabled} onChange={(event) => onChange({ ...config, enabled: event.target.checked })} /><svg viewBox="0 0 24 24" aria-hidden="true"><circle className="baseline-check-circle" cx="12" cy="12" r="10" /><path className="baseline-check-mark" d="M7 12.5 L10.5 16 L17 8.5" pathLength={100} /></svg><span>启用进出基准线</span></label>
